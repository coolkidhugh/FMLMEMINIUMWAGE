...
APP_VERSION = "2.6.0" # 版本号
APP_NAME = "金陵工具箱" # 工具箱名字
...
CTRIP_AUDIT_COLUMN_MAP_CTRIP = {
    '订单号': ['订单号', '订单号码'],
    '确认号': ['确认号', '酒店确认号', '确订号'], # 操，把“确订号”加进来了
    '客人姓名': ['客人姓名', '姓名', '入住人', '宾客姓名'],
    '到达': ['到达', '入住日期', '到店日期'],
    '离开': ['离开', '离店日期']
}
CTRIP_AUDIT_COLUMN_MAP_SYSTEM = {
    '预订号': ['预订号', '预定号'], # 操，把“预定号”加进来了
...
PROMO_CHECKER_COLUMN_MAP = {
    '订单号': ['确认号', '订单号', '预订号', '预定号'], # 操，这里的名字也要认全
    '备注': ['备注', 'Remark'],
    '房类': ['房类', '房型', 'Room Type']
}

# --- [美团邮件审核] 配置 ---
# 操，这里告诉程序，系统订单Excel里，我们要找的列可能叫啥名字
MEITUAN_SYSTEM_COLUMN_MAP = {
    '预订号': ['预订号', '预定号'], # 操，这个是用来匹配JLG号码的关键列
    '姓名': ['姓名', '名字', '客人姓名', '宾客姓名'],
    '到达': ['到达', '入住日期', '到店日期'],
    '离开': ['离开', '离店日期'],
    '第三方预定号': ['第三方预定号', '第三方预订号'], # 操，这个好像美团用不到，但先留着
    '状态': ['状态', 'Status'], # 操，新加的列
    '房号': ['房号', 'Room No', '房间号'] # 操，新加的列
}
